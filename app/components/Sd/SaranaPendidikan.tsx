...
import { SARANAPENDIDIKAN } from "../../../constants";
import Image from "next/image";

const SaranaPendidikan = () => {
  const description = `Menyiapkan Generasi masa depan yang mampunyai Imtaq dan Iptek, berakhlak mulia, berdisiplin dan mandiri.`;
  return (
    <section className="flex flexCenter overflow-hidden bg-feature-bg bg-center bg-no-repeat py-24">
      <div className=" max-container padding-container relative w-full flex flex-col justify-center items-center">
        ...
        <div className="flex w-full flex-col flexCenter">
          <ul className="mt-10 grid gap-5 md:grid-cols-3 lg:mg-2 lg:gap-20">
            {SARANAPENDIDIKAN.map((feature) => (
              <FeatureItem
                key={feature.title}
                image={feature.image}
                title={feature.title}
  ...
  return (
    <li className="mb-5 lg:mb-0">
      <div className=" flex w-full flex-1 flex-col items-center">
        <div className="relative mb-6 overflow-hidden rounded-lg bg-cover bg-no-repeat shadow-lg dark:shadow-black/20 h-[250px] sm:h-[200px] md:h-[250px] lg:h-[400px]">
