...
import { EKSTRAKURIKULER, SARANAPENDIDIKAN } from "../../../constants";
    ...
    <section className="flex flexCenter overflow-hidden bg-feature-bg bg-center bg-no-repeat py-24">
      <div className=" max-container padding-container relative w-full flex flex-col justify-center items-center">
        ...
        <div className="flex w-full flex-col flexCenter">
          <ul className="mt-10 grid gap-5 grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 lg:mg-2 lg:gap-10">
            {EKSTRAKURIKULER.map((feature) => (
              <FeatureItem
                key={feature.title}
                image={feature.image}
                title={feature.title}
